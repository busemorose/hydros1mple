...
#' score_seq(KarstMod_dataset$date, KarstMod_dataset$QobsS + runif(365, -2, +10), KarstMod_dataset$QobsS)

score_seq <- function(date, sim, obs, freq = c("hyear", "month"),
                       month_hyear = 9, lf_quantile = 0.1, hf_quantile = 0.9,
                       crit = c("NSE", "KGE", "rpearson", "rspearman", "KGE_m", "KGE_m2", "KGENP", "LME", "LCE"),
                       sf = c(1, 1, 1),
                       rel_heights = c(1, 0.9, 1.55)) {

  # Plot function
  q_plot <- function(df, freq) {

    ggplot(df, aes(!!sym(freq), score, fill = mean)) +
      ...
      {if (freq == "hyear") scale_x_continuous(breaks = seq(floor(min(df$hyear, na.rm = TRUE) / 10) * 10,
                                                            ceiling(max(df$hyear, na.rm = TRUE) / 10) * 10, 5))} +
      {if (freq == "month") scale_x_continuous(breaks = seq(1, 12, 1),
                                               labels = seq(1, 12, 1))} +
  ...
  x_mean <- x_all |> dplyr::group_by(!!sym(freq)) |> dplyr::summarise(mean = mean(obs, na.rm = TRUE))
  ...
  score <- dplyr::bind_rows(x_all |> dplyr::mutate(flow_type = "all"), x_l, x_h) |>
    dplyr::group_by(!!sym(freq), flow_type) |>
  ...
  x <- score |>
    ...
    dplyr::left_join(x_mean, by = freq)

  plot_h <- q_plot(x, freq) +
    ylab(paste0(crit, "\n", ">Q", hf_quantile * 100)) +
  ...
  plot_all <- q_plot(x, freq) +
    {if (freq == "hyear") xlab(paste0("Hydrological year (", month.name[month_hyear], ")"))} +
    {if (freq == "month") xlab("Month")} +
    ylab(paste0(crit, "\n", "Q")) +
    theme_bw(base_size = 16) +
